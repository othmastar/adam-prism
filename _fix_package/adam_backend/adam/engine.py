...
class AdamEngine:
    ...
    async def chat(
        ...
        # call LLM (Ollama أولاً، ثم mock كـ fallback أخير)
        response_text = None
        used_backend = None
        tokens_in = estimate_messages_tokens(messages)

        logger.info(f"🔁 Pipeline: memory_ltm={bool(ltm_context)} history={len(history)} messages={len(messages)} → calling Ollama ({self.config.ollama_model})")
        try:
            response_text = await self._call_ollama(messages)
            used_backend = "ollama"
            self._stats["ollama_calls"] += 1
            if response_text:
                logger.info(f"✅ Ollama succeeded: backend={used_backend}")
            else:
                logger.warning("⬇️ Ollama returned None → using Mock fallback")
        except Exception as e:
        ...
        if not response_text:
            ...
            used_backend = "mock"
            logger.info(f"✅ Mock fallback: backend={used_backend}")
    ...
    async def _call_ollama(self, messages: list[dict]) -> str | None:
        """يستدعي Ollama. يدعم gemma4 (thinking field) والموديلات العادية (content field)."""
        ...
        logger.info(f"⚡ _call_ollama → model={model}, messages={len(messages)}, timeout={self.config.ollama_timeout}s")
        ...
        if resp.status_code == 200:
            data = resp.json()
            # gemma4 يكتب الرد في thinking مش content
            content = data.get("message", {}).get("content", "").strip()
            thinking = data.get("message", {}).get("thinking", "").strip()
            logger.info(f"  ← status=200, content_len={len(content)}, thinking_len={len(thinking)}, done_reason={data.get('done_reason','')}")

            # استخدم thinking لو content فاضي (gemma4 behavior)
            if not content and thinking:
                content = thinking
                logger.info("  ← استخدمنا thinking field بدل content (gemma4)")
            ...
            logger.warning(f"  ← content فارغ (thinking كان له {len(thinking)} حرف)")
            return None
    ...
    async def health_check(self) -> dict:
        ...
        optional_services["embeddings"] = emb is True or emb is None  # None = not checked yet

        services = {**core_services, **optional_services}
        if all(core_services.values()):
            status = "healthy"
