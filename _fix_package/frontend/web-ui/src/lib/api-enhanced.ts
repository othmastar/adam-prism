...
export async function getEngineHealth() {
  return request<any>("/api/engine/health/full");
}

export async function getDiagnostics() {
  return request<any>("/api/engine/stats");
}
...
export async function fetchSession(sessionId: string): Promise<{
  session: Session;
  stats: { message_count: number; total_tokens_in: number; total_tokens_out: number };
}> {
  return request(`/api/sessions/${sessionId}`);
}

export async function createSession(title?: string): Promise<string> {
  // الإنشاء يتم تلقائياً عند أول رسالة chat
  return "";
}
...
export async function recallMemory(
  ...
  return request(
    `/api/memory/search?query=${encodeURIComponent(query)}&top_k=${limit}`,
  );
}
...
// ============================================================
// LTM — Long-Term Memory (Vector embeddings) — mapped to /api/memory/*
// ============================================================
...
export async function searchLTM(
  ...
  return request(`/api/memory/search?${params}`);
}
...
export async function recallLTMContext(
  ...
  return request(
    `/api/memory/search?query=${encodeURIComponent(query)}&top_k=${maxMemories}`,
  );
}
...
export async function getLTMHealth(): Promise<any> {
  return request("/api/engine/health/full");
}

export async function deleteLTM(memoryId: number): Promise<{ deleted: boolean }> {
  // Not supported on this backend version
