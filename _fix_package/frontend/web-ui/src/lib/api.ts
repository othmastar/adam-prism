...
export async function uploadFile(
  file: File
): Promise<{ filename: string; original_name: string; url: string; content_type: string; size: number; text_content: string } | null> {
  const url = getFastApiUrl();
  const form = new FormData();
  form.append("file", file);
  try {
...
export function useChatWebSocket(callbacks: WsCallbacks) {
  ...
  const send = React.useCallback((message: string, context?: Record<string, unknown>) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ message, voice: true, context }));
      return true;
    }
    return false;
  }, []);
