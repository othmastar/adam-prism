...
export type ViewType = "chat" | "knowledge" | "notebook" | "tools" | "settings" | "monitor" | "pipeline" | "scheduler" | "plugins" | "subagents" | "memory" | "skills" | "channels" | "ltm" | "browser" | "mcp";
...
type AppState = {
  ...
  // Guest attempt tracking (3 free attempts)
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  // Guest attempt tracking (3 free attempts)
  guestAttempts: typeof window !== "undefined" ? (() => { try { const v = localStorage.getItem("adam_attempts"); return v ? parseInt(v) : 0; } catch { return 0; } })() : 0,
  incrementGuestAttempts: () => set((s) => { const next = s.guestAttempts + 1; try { localStorage.setItem("adam_attempts", String(next)); } catch {} return { guestAttempts: next }; }),
  resetGuestAttempts: () => { try { localStorage.removeItem("adam_attempts"); } catch {} set({ guestAttempts: 0 }); },
