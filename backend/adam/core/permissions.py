"""
Adam Prism — Permission Manager (Phase 1b) — HARDENED v3
=========================================================
يدير صلاحيات تنفيذ الأدوات حسب التصنيف والمستوى.

[SECURITY FIXES v3]
1. الأذونات الخطيرة أصبحت "always-ask" بدلاً من "auto-allow"
2. إضافة تصنيفات أمنية واضحة مع مستويات الخطورة
3. دعم وضع الإنتاج (ADAM_PRODUCTION=1) يغلّظ الصلاحيات تلقائياً
4. [FIX v3] مسار دائم للسجل بدلاً من /tmp — البيانات لا تضيع عند إعادة التشغيل
5. [FIX v3] إضافة تصنيفات: workflow, admin, network
6. [FIX v3] rotation تلقائي لملف السجل
"""
...
# [FIX v3] مسار دائم بدلاً من /tmp — لا يضيع عند إعادة التشغيل
_PERMISSIONS_DIR = Path(os.environ.get(
    "ADAM_PERMISSIONS_DIR",
    os.path.expanduser("~/.adam_prism/logs")
))
_PERMISSIONS_DIR.mkdir(parents=True, exist_ok=True)
PERMISSION_LOG = _PERMISSIONS_DIR / "permissions.jsonl"

# هل نحن في وضع الإنتاج؟
IS_PRODUCTION = os.environ.get("ADAM_PRODUCTION", "0") == "1"

# تصنيفات الصلاحيات والتوصيف
# [FIX v3] الأذونات الخطيرة أصبحت "always-ask" في كل الأوضاع
...
PERMISSION_CATEGORIES = {
    ...
    # [NEW v3] تصنيفات إضافية
    "workflow": {
        "default": "ask-once",
        "description": "تنفيذ سير عمل متعدد الخطوات",
        "risk": "medium"
    },
    "admin": {
        "default": "always-ask",
        "description": "عمليات إدارية (إضافة MCP، تغيير إعدادات)",
        "risk": "critical"
    },
    "network": {
        "default": "ask-once",
        "description": "طلبات شبكة خارجية",
        "risk": "medium"
    },
}
...
TOOL_CATEGORY_MAP = {
    ...
    "web_search": "browser",
    # [NEW v3] تصنيفات إضافية
    "add_mcp_server": "admin",
    "remove_mcp_server": "admin",
    "change_config": "admin",
    "spawn_subagent": "subagent",
    "remove_subagent": "subagent",
...
def _rotate_log_if_needed():
    """[NEW v3] تدوير ملف السجل لو حجمه كبير"""
    try:
        if PERMISSION_LOG.exists() and PERMISSION_LOG.stat().st_size > 10 * 1024 * 1024:  # 10MB
            rotated = PERMISSION_LOG.with_suffix(f".{datetime.now().strftime('%Y%m%d')}.jsonl")
            if not rotated.exists():
                PERMISSION_LOG.rename(rotated)
    except Exception:
        pass


def log_permission(action: str, tool: str, category: str, reason: str, level: str, verdict: str):
    """تسجيل قرار صلاحية — [FIX v3] في مسار دائم مع rotation"""
