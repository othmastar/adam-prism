...
from adam_v3.memory import MemoryStore
from adam_v3.tools import ToolDispatcher, ToolResult
...
class AdamEngine:
    ...
    async def _call_lora(self, messages: list[dict]) -> str | None:
        ...
        logger.info(f"LoRA call: {len(messages)} msgs, {total_chars} chars, max={self.config.lora_max_chars}")
        ...
        if total_chars > self.config.lora_max_chars:
            ...
            logger.info(f"After truncation: {sum(len(m.get('content', '')) for m in messages)} chars")

        for attempt in range(self.config.lora_max_retries):
            try:
                logger.info(f"LoRA attempt {attempt+1}/{self.config.lora_max_retries}")
                resp = await client.post(
                ...
                logger.info(f"LoRA raw response length: {len(raw)}")
                if not raw:
                    error_msg = data.get("error", "")
                    logger.warning(f"LoRA returned empty response. Flask error: {error_msg}")
                    if error_msg:
                ...
                result = clean.strip() or raw.strip()
                logger.info(f"LoRA success: {len(result)} chars")
                return result
            except (httpx.TimeoutException, httpx.ConnectError) as e:
                last_exc = e
                logger.warning(f"LoRA attempt {attempt+1} timeout/connect: {e}")
                if attempt < self.config.lora_max_retries - 1:
                    await asyncio.sleep(0.5 * (2 ** attempt))
            except Exception as e:
                logger.error(f"LoRA error: {type(e).__name__}: {e}")
        ...
        if last_exc:
            ...
            return f"[Error connecting to LLM after {self.config.lora_max_retries} attempts: {last_exc}]"
        return None
