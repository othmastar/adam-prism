...
async def api_status() -> dict[str, Any]:
    ...
    # تحقق من engine_ready بشكل فعلي
    engine_ready = False
    engine_type = "none"
    active_model = None
    available_models = []
    ...
    if _engine is not None:
        ...
        try:
            ...
            engine_ready = stats.get("engine_ready", False)
            active_model = stats.get("active_model")
            available_models = stats.get("available_models", [])
        except Exception:
            engine_ready = True  # لو الـ engine موجود، يبقى ready
...
async def engine_health() -> dict[str, Any]:
    """Engine health check — مربوط فعلياً بالـ engine."""
    uptime = time.time() - _START_TIME

    # لو الـ engine المتكامل موجود، استخدمه
    if _engine is not None:
        try:
            ...
            return {
                **health,
                ...
                "model": stats.get("active_model") or OLLAMA_MODEL,
                "stats": stats,
                "system": {
    ...
    # fallback للـ checks القديمة
    services = {"api": True, "ollama": False, "qdrant": False, "engine": False}
    try:
    ...
    try:
        ...
        pass
    services["engine"] = services["api"] and services["ollama"]
    return {
        "status": "healthy" if all(services.values()) else "degraded",
        "uptime_sec": int(uptime),
        "services": services,
        "model": OLLAMA_MODEL,
        "engine_ready": services["engine"],
        "engine_type": "legacy",
        "system": {
            "cpu_percent": round(psutil.cpu_percent(), 1),
            "memory_percent": round(psutil.virtual_memory().percent, 1),
...
async def engine_diagnostics():
    """System diagnostics — مربوط فعلياً بالـ engine."""
    # لو الـ engine موجود، استخدم diagnostics الخاص بيه
    if _engine is not None:
        try:
            health = await _engine.health_check()
            stats = _engine.get_stats()
            ...
            summary = {
                "total": len(services),
                "passed": sum(1 for v in services.values() if v),
                "failed": sum(1 for v in services.values() if not v),
            }
            return {
                "summary": summary,
                "services": services,
                ...
                "engine_type": health.get("engine_type", "integrated"),
...
async def engine_heal():
    """Auto-heal — إصلاح ذاتي فعلي للـ engine."""
    if _engine is not None:
        try:
            results = await _engine.heal()
            # أعد فحص الـ health
            health = await _engine.health_check()
            return {
                "status": "healed" if results.get("fixed") else "checked",
                "details": results,
                "health": health,
                "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
            }
    ...
    # fallback
    results = {"checked": [], "restarted": [], "errors": []}
    try:
        async with httpx.AsyncClient(timeout=2) as c:
            r = await c.get("http://localhost:6333/collections")
            if r.status_code != 200:
                ...
                results["checked"].append("qdrant: ok")
    except Exception:
...
async def chat_upload(file: UploadFile = File(...), session_id: str = None):
    """Upload a file and return its content for analysis.

    لو session_id موجود، الملف بيتخزن في سياق الجلسة تلقائياً.
    """
    import shutil
    ...
    if ext in text_exts:
        ...
        try:
            ...
            content = f"[IMAGE: {safe_name} — tesseract غير مثبت. ثبتها: apt install tesseract-ocr]"
        except Exception as e:
            content = f"[IMAGE: {safe_name} — خطأ في OCR: {str(e)[:100]}]"
    elif ext in audio_exts or (file.content_type and file.content_type.startswith("audio/")):
        file_type = "audio"
        # لو الـ STT متاح، حول الصوت لنص
        try:
            # جرّب whisper لو متاح
            import subprocess
            whisper_result = subprocess.run(
                ["whisper", str(save_path), "--model", "base", "--language", "ar"],
                capture_output=True, text=True, timeout=120
            )
            if whisper_result.returncode == 0 and whisper_result.stdout:
                content = f"[TRANSCRIPT from {safe_name}]:\n{whisper_result.stdout.strip()}"
            else:
                content = f"[AUDIO: {safe_name}, {save_path.stat().st_size} bytes — whisper غير متاح]"
        except FileNotFoundError:
            content = f"[AUDIO: {safe_name} — ثبت whisper: pip install openai-whisper]"
        except Exception as e:
            content = f"[AUDIO: {safe_name} — خطأ: {str(e)[:100]}]"
    elif ext in video_exts or (file.content_type and file.content_type.startswith("video/")):
        file_type = "video"
        content = f"[VIDEO: {safe_name}, {save_path.stat().st_size} bytes — تحليل الفيديو مش متاح حالياً]"
    elif ext == ".pdf":
        file_type = "pdf"
        try:
            import fitz  # PyMuPDF
        ...
        file_type = "office"
        content = f"[{ext.upper()} FILE: {safe_name}, {save_path.stat().st_size} bytes — ثبت python-docx لقراءته]"

    # لو session_id موجود، خزّن الملف في سياق الجلسة
    session_attached = False
    if session_id and content:
        try:
            # خزّن في الذاكرة كـ message من نوع system
            if _engine is not None:
                await _engine.session_manager.add_message(
                    session_id, "system",
                    f"[FILE UPLOADED: {safe_name}]\n{content[:3000]}",
                    metadata={"type": "file_upload", "filename": safe_name}
                )
                session_attached = True
        except Exception as e:
            logger.warning(f"Failed to attach file to session: {e}")
...
async def voice_transcribe(file: UploadFile = File(...)):
    """تحويل الصوت لنص (STT) — يدعم whisper."""
    import shutil
    from pathlib import Path as _Path

    # احفظ الملف
    audio_dir = _Path("/tmp/adam_audio")
    ...
    try:
        ...
        result = subprocess.run(
            ["whisper", str(audio_path), "--model", "base", "--language", "ar",
             "--output_format", "txt", "--output_dir", str(audio_dir)],
            capture_output=True, text=True, timeout=120
        )
        if result.returncode == 0:
            txt_path = audio_dir / (audio_path.stem + ".txt")
            if txt_path.exists():
        ...
        return {
            "transcript": "",
            "error": f"whisper failed: {result.stderr[:200]}",
            "language": "ar",
            "confidence": 0.0,
        }
    except FileNotFoundError:
...
async def voice_synthesize(request: dict):
    ...
    # جرّب pyttsx3 (مجاناً ومحلي)
    try:
        import pyttsx3
        import tempfile

        engine_tts = pyttsx3.init()
        engine_tts.setProperty("rate", int(200 * speed))

        # احفظ في ملف
        audio_path = tempfile.NamedTemporaryFile(
            suffix=".wav", delete=False, dir="/tmp/adam_audio"
        ).name
        engine_tts.save_to_file(text, audio_path)
        ...
        return {"error": str(e)[:200], "text": text}


@app.post("/api/voice/chat", tags=["voice"])
async def voice_chat(file: UploadFile = File(...), session_id: str = None):
    """Voice chat — STT → chat → TTS pipeline."""
    # 1. حول الصوت لنص
    transcribe_result = await voice_transcribe(file)
    transcript = transcribe_result.get("transcript", "")

    if not transcript:
        return {
            "text": "",
            "error": transcribe_result.get("error", "STT failed"),
            "audioUrl": None,
            "duration_ms": 0,
        }

    # 2. ابعت النص للـ chat
    ...
    audio_url = None
    try:
        synth_result = await voice_synthesize({"text": response_text})
        if hasattr(synth_result, "body"):
            # FileResponse — احفظ URL
            audio_url = f"/api/voice/audio/tts_{int(time.time())}.wav"
    except Exception as e:
