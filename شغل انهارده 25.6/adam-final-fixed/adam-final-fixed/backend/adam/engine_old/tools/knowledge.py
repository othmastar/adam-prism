...
class KnowledgeMixin:
    ...
    async def _tool_knowledge(self, params: dict) -> dict:
        ...
        try:
            ...
            ollama_base = self.config.get("embedding_base_url") or self.config.get("ollama_base", "http://localhost:11434")
            hc = await self.shared_clients.get("ollama", ollama_base, timeout=10)
            o_resp = await hc.post("/api/embeddings", json={
                "model": self.config.get("embedding_model", "nomic-embed-text"), "prompt": query
            })
            o_data = o_resp.json()
